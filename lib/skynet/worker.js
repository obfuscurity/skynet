  ...
  if (channel == workerUuid) {
    ...
    if (jobInfo.jobType == 'map') {
      ...
      var emit = function (key, val) {
        bucket = workerNumber; //krHash(key) % jobInfo.workerCount;
        ...
        if (Math.floor(++len / jobInfo.dataLength) > pct) {
          ...
          pubClient.publish(workerUuid, JSON.stringify(statusObj));
      ...
      request.on('response', function (res) {
        ...
        res.on('end', function () {
          ...
          pubClient.publish(workerUuid, JSON.stringify({
            progress: 100,
            stage: 'map'
          }));
          pubClient.publish(workerUuid, JSON.stringify({ imFiles: imFiles }));
        });
      });
      request.end();
...
pubClient.publish('scheduler', JSON.stringify({ workerId: workerUuid }));
