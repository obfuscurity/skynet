...
var parsedDsn = url.parse(process.env.DATABASE_URL);

var pgConnStr = 'tcp://' + parsedDsn.auth + '@' + parsedDsn.host + ':' +
  parsedDsn.port + '/' + parsedDsn.path;

var job = {};
var db;

pg.connect(pgConnStr, function (err, pgClient) {
  if (err != null) {
  ...
  client.query('select * from jobs j left join workers w on w.job_id = j.id
                where w.job_id is null limit 1', function (err, res) {
    if (err != null) {
...
var scaleWorkers = function (n) {
  ...
  req = http.request(postOpts, function (res) {});
  ...
  if (channel == 'scheduler') {
    if (data.workerId != undefined) {
      if (workers[data.workerId] == undefined) {
        ...
        db.query('insert into workers
                    (uuid, type, state, progress, created_at, updated_at, job_id)
                  values
                    ($1, $2, $3, $4, $5, $6, $7)',
    ...
    if (data.splitFiles != undefined) {
      ...
      scaleWorkers(job.nodes);
      splitters = {};
    } else if (data.progress != undefined) {
      if (progress < 100) {
        db.query('update workers set progress=$1, updated_at=now()
                  where uuid=$2 and job_id=$3 and type=$4',
          [ data.progress, channel, job.id, 'splitter' ]);
      } else {
        db.query('update workers set progress=$1, state=$2, updated_at=now()
                  where uuid=$3 and job_id=$4 and type=$4',
    ...
    if (data.imFiles != undefined) {
      ...
      if (progress < 100) {
        if (workers[channel].stage == 'map') {
          db.query('update workers set progress=$1, updated_at=now()
                    where uuid=$2 and job_id=$3 and type=$5',
        ...
        if (workers[channel].stage == 'map') {
          ...
          db.query('update workers set progress=$1, state=$2, updated_at=now()
                    where uuid=$3 and job_id=$4 and type=$5',
